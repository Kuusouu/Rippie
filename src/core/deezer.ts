...
const DIRECT_TRACK_ID_PATTERN =
	/(?:www\.)?deezer\.com\/(?:\w{2}\/)?track\/([0-9]+)/;
...
export const extractDeezerTrackId = async (
	url: string,
): Promise<string | null> => {
...
export const fetchDeezerTrackInfo = async (
	trackId: string,
): Promise<TrackInfo> => {
	...
	if (!response.ok) {
		throw new Error(
			`Deezer track fetch failed: ${response.status} ${response.statusText}`,
		);
	}
	...
	if (track.error) {
		throw new Error(
			`Deezer API error: ${track.error.message} (code ${track.error.code})`,
		);
	}
...
export const lookupDeezerTrackByIsrc = async (
	isrc: string,
): Promise<DeezerTrackLookup> => {
	...
	if (!response.ok) {
		throw new Error(
			`Deezer ISRC lookup failed: ${response.status} ${response.statusText}`,
		);
	}
